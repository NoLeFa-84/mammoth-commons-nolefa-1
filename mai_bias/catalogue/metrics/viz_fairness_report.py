...
def viz_fairness_report(
    ...
    about_html = f"""
        <p>This report assesses whether the model behaves similarly across different population groups,
        as defined by the following sensitive attributes: {sensitive_list_html}</p>
        <p>Two types of fairness metrics are evaluated:
        <ul>
            <li><strong>Group-wise metrics</strong>: Per-group performance values (e.g., true positive rates per group).</li>
            <li><strong>Scalar metrics</strong>: Single-value summaries of disparity (differences close to 0 and ratios close to 1 indicate balanced treatment).</li>
        </ul></p>
    ...
    # --- Generate the full Fairlearn HTML table (goes into experts panel) ---
    ...
    return HTML(
        simplified_formatter(
            ...
            methodology=f"""
                ...
                Group-wise metrics are computed independently per group, and scalar metrics aggregate these into summary
                statistics that capture the degree of disparity across groups.</p>
